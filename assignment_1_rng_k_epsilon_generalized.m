...
%              flow using RNG k-ε model

clc; close all; clear;
image_type = 'epsc';

% Open files for data loggging
file = fopen("solution_rng_k_epsilon.txt", "w");
file1 = fopen('simulated_data_rng_k_epsilon.dat', 'w');
...
fprintf('|                               flow using RNG k-ε model                            |\n');
fprintf('*-----------------------------------------*---------------------------------------- *\n\n');
...
c1_eps = zeros(ny, 1); % RNG k-ε model constant
U = zeros(ny, 1);
dUdy = ones(ny-1, 1);
k = 10*ones(ny, 1);
eps = 10*ones(ny, 1);
...
while (max(res) > residue_limit)
    ...
    % Calculating C1 (for RNG k-ε model)
    for i=2:ny-1
        eta = abs(dUdy(i))*k(i)/eps(i);
        c1_eps(i) = 1.42 - (eta*(1-eta/eta_0) / (1+beta*eta^3));
    end
    ...
    for i=2:ny-1
        if (i == 2)
            ...
            ap(i) = an(i) - sp(i);
            su(i) = c1_eps(i)*Pk(i)*eps(i)*(y_surface(i)-y_surface(i-1))/k(i);
        elseif (i == ny-1)
...
ylabel('y/h');
title('U-velocity');
legend('DNS', 'RNG k-ε', 'Location', 'northwest');
legend boxoff;
saveas(1, './Plots/rng_k_epsilon/U_velocity_plot', image_type);
...
legend('DNS', 'RNG k-ε', 'Location', 'northeast');
legend boxoff;
saveas(2, './Plots/rng_k_epsilon/TKE_plot', image_type);
...
legend('DNS', 'RNG k-ε', 'Location', 'northeast');
legend boxoff
saveas(3, './Plots/rng_k_epsilon/Dissipation_rate_of_k_plot', image_type);
...
legend('DNS', 'RNG k-ε', 'Location', 'northeast');
legend boxoff
saveas(4, './Plots/rng_k_epsilon/Turbulent_shear_stress_plot', image_type);
...
legend('DNS', 'RNG k-ε', 'Location', 'northeast');
legend boxoff
saveas(5, './Plots/rng_k_epsilon/Pk_plot', image_type);
...
legend('DNS', 'RNG k-ε', 'Location', 'northeast');
legend boxoff
saveas(6, './Plots/rng_k_epsilon/Turbulent_diffusion_of_k_plot', image_type);
...
legend boxoff
saveas(7, './Plots/rng_k_epsilon/Viscous_diffusion_of_k_plot', image_type);
...
xlabel('yᐩ');
title('Budget for k (RNG k-ε model)');
legend('Dissipation rate of k', 'Production rate of k', ...
    'Turbulent diffusion of k', 'Viscous diffusion of k', 'Location', 'northeast');
legend boxoff
saveas(9, './Plots/rng_k_epsilon/Budget_for_turbulence_(rng_k_epsilon)', image_type);
