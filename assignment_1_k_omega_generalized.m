...
%              flow using k-ω model

clc; close all; clear;
image_type = 'epsc';

% Open files for data loggging
file = fopen("solution_k_omega.txt", "w");
file1 = fopen('simulated_data_k_omega.dat', 'w');

fprintf('*-----------------------------------------*-----------------------------------------*\n');
fprintf('|                                 Author: Nirupam Pal                               |\n');
fprintf('|                                   Date: 19/03/2024                                |\n');
fprintf('|     Description: This program is used for solving the fully developed channel     |\n');
fprintf('|                                 flow using k-ω model                              |\n');
fprintf('*-----------------------------------------*---------------------------------------- *\n\n');
...
omega_res = 2*residue_limit; % Initial ω residual
res = [U_res; k_res; omega_res];
...
while (max(res) > residue_limit)
    ...
    if (iteration < 100)
        ...
        nu_t(ny) = nu_t(ny-1);
    % Solving for eddy-viscosity using k-ω model
    else
        for i=2:ny-1
            nu_t(i) = k(i) / omega(i);
        end
    ...
    for i=2:ny-1
        if (i == 2)
            ...
            sp(i) = -(nu + nu_t_surface(i-1)/sigma_k) / dy(i-1) - beta_star*omega(i)*(y_surface(i)-y_surface(i-1));
            ap(i) = an(i) - sp(i);
            su(i) = (y_surface(i)-y_surface(i-1))*Pk(i) + (nu + nu_t_surface(i-1)/sigma_k) * k(1) / dy(i-1);
        elseif (i == ny-1)
            as(i) = (nu + nu_t_surface(i-1)/sigma_k) / dy(i-1);
            sp(i) = -beta_star*omega(i)*(y_surface(i)-y_surface(i-1));
            ap(i) = as(i) - sp(i);
    ...
    % Creating the ω equations for the grid
    omega_old = omega;
    for i=2:ny-1
        if (i == 2)
            an(i) = (nu + nu_t_surface(i)/sigma_omega) / dy(i);
            sp(i) = -c2_omega*omega(i)*(y_surface(i)-y_surface(i-1));
            ap(i) = an(i) - sp(i);
            su(i) = c1_omega*Pk(i)*omega(i)*(y_surface(i)-y_surface(i-1))/k(i);
        elseif (i == ny-1)
            as(i) = (nu + nu_t_surface(i-1)/sigma_omega) / dy(i-1);
            sp(i) = -c2_omega*omega(i)*(y_surface(i)-y_surface(i-1));
            ap(i) = as(i) - sp(i);
            su(i) = c1_omega*Pk(i)*omega(i)*(y_surface(i)-y_surface(i-1))/k(i);
        else
    ...
    % Enforcing the given ω boundary condition at yᐩ ≤ 3
    for i=2:m
        omega(i) = 6 * nu / (c2_omega*y_dns(i)^2);
    end
    
    % Calculating the ω
    omega = gauss_seidel(as, ap, an, su, omega, m+1, ny-1);
    omega(1) = omega(2);
    omega(ny) = omega(ny-1);

    % Calculating ω using relaxation parameter
    omega = (1-urf)*omega_old + urf*omega;

    % Calculating the residuals
    res(1) = norm(U_old - U);
    res(2) = norm(k_old - k);
    res(3) = norm(omega_old - omega);

    % Printing the residuals
    fprintf(file, 'Iteration: %d\n', iteration);
    fprintf(file, 'U residual: %.4f\n', res(1));
    fprintf(file, 'k residual: %.4f\n', res(2));
    fprintf(file, 'ω residual: %.4f\n\n', res(3));
    fprintf('Iteration: %d\n', iteration);
    fprintf('U residual: %.4f\n', res(1));
    fprintf('k residual: %.4f\n', res(2));
    fprintf('ω residual: %.4f\n\n', res(3));
end
...
ylabel('y/h');
title('U-velocity');
legend('DNS', 'k-ω', 'Location', 'northwest');
legend boxoff;
saveas(1, './Plots/k_omega/U_velocity_plot', image_type);
...
legend('DNS', 'k-ω', 'Location', 'northeast');
legend boxoff;
saveas(2, './Plots/k_omega/TKE_plot', image_type);
...
legend('DNS', 'k-ω', 'Location', 'northeast');
legend boxoff
saveas(3, './Plots/k_omega/Dissipation_rate_of_k_plot', image_type);
...
legend('DNS', 'k-ω', 'Location', 'northeast');
legend boxoff
saveas(4, './Plots/k_omega/Turbulent_shear_stress_plot', image_type);
...
legend('DNS', 'k-ω', 'Location', 'northeast');
legend boxoff
saveas(5, './Plots/k_omega/Pk_plot', image_type);
...
legend('DNS', 'k-ω', 'Location', 'northeast');
legend boxoff
saveas(6, './Plots/k_omega/Turbulent_diffusion_of_k_plot', image_type);
...
legend('DNS', 'k-ω', 'Location', 'northeast');
legend boxoff
saveas(7, './Plots/k_omega/Viscous_diffusion_of_k_plot', image_type);
...
xlabel('yᐩ');
title('Budget for k (k-ω model)');
legend('Dissipation rate of k', 'Production rate of k', ...
    'Turbulent diffusion of k', 'Viscous diffusion of k', 'Location', 'northeast');
legend boxoff
saveas(9, './Plots/k_omega/Budget_for_turbulence_(k_omega)', image_type);
